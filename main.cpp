...
HWND		g_hWnd;			// 윈도우 핸들
LPSTR		g_lpszClass = (LPSTR)TEXT("Pokemon_Shooting");
	...
	LPSTR _lpszCmdParam, int nCmdShow)
{
	...
	while (true)
	{
		if (PeekMessage(&message, 0, 0, 0, PM_REMOVE)) //메시지가 발생하면 처리하고 메시지가 없으면 다른 로직 처리하게 만들어 줄것.
		{
		...
		{
			TimeManager::GetSingleton()->Update(60.0f);
	...
	//while (GetMessage(&message, 0, 0, 0)) //GetMessage 메시지가 발생할때까지 기다리고 있는다. 그래서 발생하면 그때 처리.
	//{
	//	TranslateMessage(&message);
	//	DispatchMessage(&message);
	//}
