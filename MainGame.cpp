	...
	if (isStart)
	{
		...
		if (ash->GetGameOver())
		{
			if (KeyManager::GetSingleton()->IsOnceKeyUp(VK_UP))
			{
				...
				continueCount--;
				if (continueCount < 0) continueCount = 1;
			...
			{
				continueCount++;
				if (continueCount > 1) continueCount = 0;
			}
			if (KeyManager::GetSingleton()->IsOnceKeyDown('Z'))
			{
				...
				if (continueCount==0)
				{
					ash->SetContinueCount(continueCount);
					continueCount = 0;
				}
				else
				{
...

	{
		...
		if (ash)
		{
			if (ash->GetGameOver())
			{
				...
				if (!hasChoice)
					ui->PrintContinue(backBuffer->GetMemDC(), continueCount);//continueCount);
