...
HRESULT Phantom::Init(float posX, float posY)
{
	isDie = false;
	isHit = false;
	...
	ImageManager::GetSingleton()->AddImage("ÆÒÅÒ", "Image/Phantom.bmp", pos.x, pos.y, 84, 22, 3, 1, true, RGB(255, 0, 255));
	image = ImageManager::GetSingleton()->FindImage("ÆÒÅÒ");
...
void Phantom::Update()
{
	if (!isDie)
	{
		...
		if (pos.x >= WINSIZE_X +30) isDie = true;
		if (HP>0)
		{
			fireTime++;
...
void Phantom::Render(HDC hdc)
{
	if (!isDie)
	{
		if (image)
		{
			image->AlphaRender(hdc, pos.x, pos.y, currentFrameX, currentFrameY, alpha, 2, degree, false);
		}
...
void Phantom::SetAsh(Ash* _ash)
{
	ash = _ash;
}

void Phantom::CheckCrash(bool* ishit, bool* onhit)
{
...
void Phantom::Reset(float posX, float posY)
{
	isDie = false;
	isHit = false;
